...
public class FileTransferController {
    ...
    public ApiDefaultResponse base64UpLoad( HttpServletRequest request, @RequestBody UploadFileRequest uploadFileRequest) throws Exception{
		List<String> fileUploadedUrls = new ArrayList<String>();
		fileUploadedUrls = this.fileTransferService.base64UpLoad(request, uploadFileRequest);
		if(LOG.isDebugEnabled()){
			LOG.debug("成功上传图片 {} 个", fileUploadedUrls.size());
		}
        return new ApiDefaultResponse(ConstantFields.getSuccessCode(), ConstantFields.getSuccessMsg(), fileUploadedUrls);
    }
